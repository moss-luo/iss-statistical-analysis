...
function datasort(id){
	var datasortID=$(id+" thead th");
	for(i=0; i<datasortID.length; i++){
	jQuery(datasortID[i]).attr("onclick","datasortonclick('"+i+"','"+id+" thead th')");
	}	
}		

function datasortonclick(e,datasortth){
	var datasort=$(datasortth);
	var datasortfontID=jQuery(datasort[e]).children("div").children("font").attr("id");
	jQuery(datasort).children("div").children("font").remove();
	if(datasortfontID){
		if(datasortfontID==='Isort'+e){
			jQuery(datasort[e]).children("div").append("<font style='position:absolute; top:-2px; font-size:15px;' id='Psort"+e+"'>&nbsp;↑</font>");
		}else{
		jQuery(datasort[e]).children("div").append("<font style='position:absolute; top:-2px; font-size:15px;' id='Isort"+e+"'>&nbsp;↓</font>");
	}
	}else{
		jQuery(datasort[e]).children("div").append("<font style='position:absolute; top:-2px; font-size:15px;' id='Isort"+e+"'>&nbsp;↓</font>");
	}
...
function getTickInterval(info){
	...
	var cdy=info.length;
	var tickInterval=Math.ceil(cdy/29);
	if(tickInterval>3){
...
function timePeriod(time){
	if(time==0){
		...
		var start=parseInt(time)-1;
		var end=parseInt(time)+1;
		if(start<0){
			start=0;
		}
		if(start>24){
